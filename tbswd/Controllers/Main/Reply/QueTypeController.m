...
#import "QueTypeController.h"

@interface QueTypeController ()
{
    NSInteger       _page;
...
@implementation QueTypeController
@synthesize source;
...
- (void)viewDidLoad
{
    ...
    if ([source isEqualToString:@"zuixinwenti"]) {
        [self setBarTitle:@"最新问题"];
    }}
...
- (void)getTableData:(NSString *)location
{
    NSDictionary    *infos;
    if ([source isEqualToString:@"zuixinwenti"]) {
         infos = @{@"MType":@"10", @"PageIndex":[NSString stringWithFormat:@"%d", _page], @"PageSize":@"10",@"IsLatestShow":@"1"};
    }
...
- (void)commentAction:(UIButton *)sender
{
    ...
    [self performSegueWithIdentifier:@"wenda_queType" sender:self];
}
